const KeyFactory = CasperStorage.KeyFactory
const EncryptionType = CasperStorage.KeyFactory
const CasperHDWallet = CasperStorage.CasperHDWallet
const casperClientSDK = casper.casperClientSDK
const Keys = casper.Keys
const CasperClient = casper.CasperClient
const CLPublicKey = casper.CLPublicKey
const DeployUtil = casper.DeployUtil
const CLValueBuilder = casper.CLValueBuilder
const RuntimeArgs = casper.RuntimeArgs
const ERC20Client = casper.ERC20Client
const constants = casper.constants
const bip39 = casper.bip39
const Buffer = casper.Buffer
...
const getAnyBalance = async (address) => {
  ...
  if (publicAccount) {
    publicAccount.balance =
      (await RPC.getAccountBalance(
        await getStateHash(),
        publicAccount.main_purse
...
const casperClientAccess = {
  ...
  mnemonic: {
    generate: async (language) => {
      ...
      return {
        mnemonic: mnemonic,
        seed: await casperClientAccess.seed.getFromMnemonic(mnemonic, language),
      }
    },
  ...
  getPublicKey: async (seed, index) =>
    (await getAccount(seed, index, true)).public, // get the private key
  getAddress: async (seed, index) => (await getAccount(seed, index)).address, // get the address
  getAnyBalance: getAnyBalance,
  ...
  tx: async (seed, index, destination, amount, erc20Contract) => {
    // send a transfer
    try {
      if (erc20Contract) {
        ...
        console.log(await getAccount(seed, index, true))
        return sendTransfer(
          new Date().getTime(),
          'casper' + (TESTNET ? '-test' : ''),
