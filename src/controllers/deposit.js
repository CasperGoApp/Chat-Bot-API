...
module.exports = async (CONTROLLERS, coinUser, wallets, searchArray) => {
  // export module
  let foundCoin = null // found coin
  let returnQR = null // return qr
  let onlyPlus = false // only plus
  let showAll = false // show all
  for (let i = 0; i < searchArray.length && !returnQR; i++) {
    // for
    if (['qr', 'code'].includes(searchArray[i].toLowerCase())) {
      // if
      returnQR = true // return qr
    }
  }
  for (let i = 0; i < searchArray.length && !onlyPlus; i++) {
    // for
    if (['+', 'plus'].includes(searchArray[i])) {
      // if
  ...
  for (let x in coinList) {
    // for
    for (let i = 0; i < searchArray.length && !foundCoin; i++) {
      // for
      onlyPlus = onlyPlus || searchArray[i].substr(-1) == '+' // only plus
      if (coinList[x].includes(searchArray[i].toLowerCase())) {
        // if
  ...
  if (wallets.length == 1) {
    ...
    await CONTROLLERS.messages.send.text(
      // send text
    ...
    if (returnQR) {
      // if
      let qr_svg = qr.imageSync(wallets[0].coin + ':' + wallets[0].address, {
        // qr svg
        type: 'png', // type
      }) // qr svg
