...
module.exports = async (CONTROLLERS, coinUser, message) => {  // export module
  const account = await CONTROLLERS.mgo.singleQuery(CONTROLLERS.db, 'Wallets', {  // get account
  ...
  if (account) {  // check account
    const earnings = await CONTROLLERS.mgo.query( // get earnings
      CONTROLLERS.db, // db
    ...
    let threeMonth = { on: new Date(), staking: 0, lottery: 0, ror: 0, rors: 0 }  // three month
    ...
    for (let i = 0; i < earnings.length; i++) { // for earnings
      ...
      if (earnings[i].on > threeMonth.on) { // check on
      ...
      if (earnings[i].on > oneYear.on) {  // check on
      ...
      if (earnings[i].on > lifetime.on) { // check on
    ...
    await CONTROLLERS.messages.message( // send message
      ...
      [
        ...
        lifetime.staking > 0  // lifetime staking
          ? (oneWeek.staking > 0  // one week staking
              ? '\n' +  
                CONTROLLERS.prices.formatCrypto(  // format crypto
                  'divi', // coin
