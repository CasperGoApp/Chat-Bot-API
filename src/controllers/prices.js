...
const prices = {  // prices
  ...
  getPrices: async _ => { // get prices
    try { // try
      const newPrices = await ( // new prices
        await APP.http(true, 'gnome.one', 'get', 443, {}, '/priceUSD', {})  // get price usd
      ).json()  // json
      if (  // if
  ...
  fiatToCrypto: (coin, amount, fiat) => { // fiat to crypto
  ...
  formatAmount: (coin, fiat) => { // format amount
    ...
    return (  // return
      ...
      APP.addCommas(  // add commas
        fiatAmount.toFixed( // to fixed
  ...
  formatCrypto: (coin, amount, fiat) => { // format crypto
    amount = amount ? amount : 0  // amount
    let fiatAmount = 0  // fiat amount
    if (fiat.toLowerCase() == 'usd') {  // if
      if (prices.prices[coin.toLowerCase()]) {  // if
  ...
  convertCrypto: (coin, amount, fiat) => {  // convert crypto
    amount = amount ? amount : 0 // amount
    let fiatAmount = 0  // fiat amount
    if (fiat.toLowerCase() == 'usd') {  // if
      if (prices.prices[coin.toLowerCase()]) {  // if
  ...
  getCurrencyPriceList: cur => {  // get currency price list
    let list = [] // list
