...
module.exports = async (CONTROLLERS, wallet, message, skipActions) => { // export module
  ...
  if (coinUser) { // check user
    ...
    const isPending = // check pending
      ...
      5 * 60 * 1000 // 5 min
    if (message.text && message.text.substr(0, 1) == '/') { // if text
    ...
    const buff = message.text // buff
      ? APP.replaceAll('\n', ' ', message.text) // replace all
          ...
          .split(' ') // split
      : []  // if not text
    if (isPending) {  // if pending
      for (let i = 0; i < buff.length; i++) { // for buff
        if (buff[i].toLowerCase() == coinUser.pending.toLowerCase()) {  // if buff
          buff.splice(i, 1) // buff
        } 
      }
    }
    let wallets = await CONTROLLERS.mgo.query(  // get wallets
      CONTROLLERS.db, // db
      'Wallets',  // collection
    ...
    if (  // check keyword
      ...
      switch (isPending ? coinUser.pending : keyword) { // switch
        case 'wallet':  // wallet
        ...
        case 'loud':  // loud
          ...
          break // break
        case 'send':  // send
          if (!isPending || !coinUser.pending) {  // if not pending
            buff.shift()  // shift
          } 
        ...
        case 'settings':  // settings
          ...
          const linkCode3 = await CONTROLLERS.shortLink.add(  // add short link
        ...
        case 'history': // history
          ...
          const linkCode1 = await CONTROLLERS.shortLink.add(  // add short link
            'https://caspergo.io/history/' + historyCode, // link
            3 * 60  // 3 min
          )
          https: await CONTROLLERS.messages.message(  // message
            message.route,  // route
            message.from, // from
            coinUser.language,  // language
            'history',  // text
        ...
        case 'support': // support
          const linkCode2 = await CONTROLLERS.shortLink.add(  // add short link
        ...
        case 'paymentrequest':  // payment request
          if (buff[0].toLowerCase().trim() == 'yes') {  // if yes
            coinUser.sendTo = { // send to
              coinName: coinUser.paymentRequest.coin.toLowerCase(), // coin name
              amount: coinUser.paymentRequest.amount  // amount
            }
            const sourceAccount = await CONTROLLERS.mgo.singleQuery(  // source account
              CONTROLLERS.db, // db
              'Wallets',  // collection
              {
            ...
            const destUser = await CONTROLLERS.mgo.singleQuery(  // dest user
              CONTROLLERS.db, // db
              'Users',  // collection
              {
            ...
            const destAccount = destUser  // dest account
              ? await CONTROLLERS.mgo.singleQuery(CONTROLLERS.db, 'Wallets', {  // dest account
            ...
            if (
              ...
              await CONTROLLERS.mgo.update(CONTROLLERS.db, 'Users', { // update
                _id: CONTROLLERS.mgo.id(coinUser._id),  // search for
                paymentRequestCompleted: new Date() // payment request completed
              })
              await CONTROLLERS.banq.run('sendInternal', [ // send internal
                  coinUser, // coin user
                  destUser, // dest user
    ...
    await CONTROLLERS.banq.run('addAccount', [ // add account
      wallet, // wallet
      message.from, // from
      message.route,  // route
      'english',  // language
