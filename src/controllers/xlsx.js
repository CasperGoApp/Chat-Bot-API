...
const makeSheet = (name, data, columns) => {  // make sheet
  ...
  if (data.length > 0) {  // if data
    if (!columns) { // if columns
      for (let x in data[0]) {  // for data
        sheetData.columns.push({ label: x, value: x })  // columns
      }
    }
    for (let i = 0; i < data.length; i++) { // for data
...
module.exports = {  // export module
  ...
  makeXLSX, // make xlsx
  expressSend: (fileName, sheets, res) => { // express send
    res.writeHead(200, {  // write head
