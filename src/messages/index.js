...
const getRecents = async (time, existingUsers, field) => {
  // get recents
  ...
  for (let i = 0; !isFound && i < fieldList.length; i++) {
    // for each field
  ...
  const recentMessages = await CONTROLLERS.mgo.query(CONTROLLERS.db, 'Users', {
    // recent messages
...
const messages = {
  ...
  sendTimedUpdates: async (seconds, list) => {
    // send timed updates
    switch (
      seconds // switch
    ) {
  ...
  message: async (provider, to, language, name, args) => {
    // message
    ...
    if (languageData[name]) {
      ...
      const received = await CONTROLLERS.mgo.singleQuery(
        // received
        CONTROLLERS.db, // db
      ...
      if (
        ...
        return await messages.send.template(
          // send template
          provider, // provider
          to, // to
  ...
  send: {
    ...
    template: async (
      ...
      await providers[provider].send.template(
        // send template
        to, // to
        name, // name
